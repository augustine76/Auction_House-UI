      ...
      if (findCollection) {
        ...
        if (findMintKey.sellerWallet != publicKey) {
          if (
            ...
            var newvalues = {
              $set: { tradingVolume: tradingVolumeOfBuyingNfts },
            }
            Collection.updateOne(myquery, newvalues, function (err, res) {
              if (err) throw err
            })
            if (findCollection.totalUniqueHolders > 0) {
              ...
              console.log(
                'findNftUniqueHoldersOfCollection ===>',
                findNftUniqueHoldersOfCollection.length,
              )
              var myquery = {
                collectionName: findCollection.collectionName,
              ...
              var newvalues = {
                $set: {
                  totalUniqueHolders: user.publicKey.length,
                },
              }
              Collection.updateOne(myquery, newvalues, function (err, res) {
                if (err) throw err
            ...
            var newvalues = {
              $set: { buyerWallet: publicKey, isBuy: true },
            }
